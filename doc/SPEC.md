...
| 課題 | 優先度 | 詳細 |
|------|--------|------|
|メモの各要素が、縦スクロール許可されており、入力すると文字が見切れる|高||
|検索機能で、文字入力後に候補をタップするとキーボードが閉じるだけで候補を押せていない|高|------|
|横のスワイプでページを行き来したい|高|------|
|編集時、保存するチェックボタンをまるで囲う|高|------|
|ノートの最後の行は太く（最初の行と同じに）|高|------|
|最初の行にもテキストをいれられるように|高|------|
|テキスト入力後、属性を変化させても文字の属性が変わらない|高い|------|
